...
class BackscatterDataset(Dataset):
    def __init__(self, data_path, train=True, transform=None, individual_transform=None, seed=0, long_sequence_length=7, subsampled_sequence_length=5, with_replacement=True):
        ...
        self.out_transform = normalize#custom_transforms.Compose([custom_transforms.ArrayToTensor(), normalize])
    ...
    def __getitem__(self, idx):
        ...
        #    indices += sorted(  -np.random.choice(max(1, len(long_sequence) - 3), self.subsampled_sequence_length-1, replace=True) -1 , reverse=backward)
        #else:
        #    indices += sorted( -np.random.choice(max(1, len(long_sequence) - 3), self.subsampled_sequence_length-1, replace=False) -1, reverse=backward)
        if self.with_replacement:
            indices = sorted(  -np.random.choice(len(long_sequence), self.subsampled_sequence_length-1, replace=True) -1 , reverse=backward)
        else:
            indices = sorted( -np.random.choice(len(long_sequence), self.subsampled_sequence_length-1, replace=False) -1, reverse=backward)
        ...
        if self.transform:
            ...
            imgs_out = self.out_transform([self.totensor(img) for img in imgs])
        else:
            imgs_out, intrinsics = self.out_transform([self.totensor(img) for img in imgs])
        if self.individual_transform:
            imgs_individually_transformed = [self.individual_transform(img) for img in imgs]
            imgs_individually_transformed = self.out_transform([self.totensor(img) for img in imgs_individually_transformed])
        else:
            imgs_individually_transformed = imgs_out
        
        return imgs_out, imgs_individually_transformed
    

class CausticsDataset(Dataset):
    def __init__(self, data_path, train=True, transform=None, seed=0, long_sequence_length=7, subsampled_sequence_length=2, with_replacement=True):
        """The data_path argument points to a directory structured in the KITTI format:
        which means that it contains a subdirectory for each sequence, and each sequence
        <data_path>/<path/to/sequence_1>/0000001.jpg
        <data_path>/<path/to/sequence_1>/0000002.jpg
